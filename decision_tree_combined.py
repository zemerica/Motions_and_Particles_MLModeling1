...
best_tree_depth_m = 16
best_min_samples_leaf_m = 26
best_max_nodes_m = 75
########################################################

########## BEST FOUND PARAMETERS - Particles ###########
best_tree_depth_p = 7
best_min_samples_leaf_p = 15
best_max_nodes_p = 27
########################################################

####BASE MODEL###
model_m = DecisionTreeClassifier(criterion='gini', class_weight='balanced', max_depth = best_tree_depth_m, min_samples_leaf=best_min_samples_leaf_m, max_leaf_nodes = best_max_nodes_m, random_state=42) 
model_p = DecisionTreeClassifier(criterion='gini', class_weight='balanced', max_depth = best_tree_depth_p, min_samples_leaf=best_min_samples_leaf_p, max_leaf_nodes = best_max_nodes_p, random_state=42) 
#####
...
if simple_run:
    t1 = time.time()
    #model = DecisionTreeClassifier(criterion='gini', class_weight='balanced', max_depth = best_tree_depth, min_samples_leaf=best_min_samples_leaf, max_leaf_nodes = best_max_nodes, random_state=42) 
...
if test_criterion:
    ...
    for n in n_range:
        ...
        model_m = DecisionTreeClassifier(criterion=n)
        ...
        model_p = DecisionTreeClassifier(criterion=n)
...
test_min_samples_leaf = False
if test_min_samples_leaf:
    test_parameter = 'Minimum Samples per Leaf'
    n_range = range(5, 100, 1) 
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = DecisionTreeClassifier(min_samples_leaf=n)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = DecisionTreeClassifier(min_samples_leaf=n)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
test_max_nodes = False
if test_max_nodes:
    test_parameter = 'Maximium Number of Nodes'
    n_range = range(5, 100, 1)
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = DecisionTreeClassifier(max_leaf_nodes=n)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = DecisionTreeClassifier(max_leaf_nodes=n)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
if test_max_depth:
    test_parameter = 'Maximium Tree Depth'
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = DecisionTreeClassifier(max_depth=n)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = DecisionTreeClassifier(max_depth=n)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
if show_validation_performance:
    ...
    best_m = DecisionTreeClassifier(criterion='gini', class_weight='balanced', max_depth = best_tree_depth_m, min_samples_leaf=best_min_samples_leaf_m, max_leaf_nodes = best_max_nodes_m, random_state=42) 
    ...
    best_p = DecisionTreeClassifier(criterion='gini', class_weight='balanced', max_depth = best_tree_depth_p, min_samples_leaf=best_min_samples_leaf_p, max_leaf_nodes = best_max_nodes_p, random_state=42) 
