...
best_estimators_m = 900
best_lr_m = 1.3
best_max_depth_m = 9
########################################################

########## BEST FOUND PARAMETERS - Particles ###########
best_estimators_p = 300
best_lr_p = 0.5
best_max_depth_p = 6
########################################################
...
####BASE MODEL###
model_m = AdaBoostClassifier(DecisionTreeClassifier(max_depth = best_max_depth_m),n_estimators = best_estimators_m, learning_rate = best_lr_m, random_state=42)
model_p = AdaBoostClassifier(DecisionTreeClassifier(max_depth = best_max_depth_p),n_estimators = best_estimators_p, learning_rate = best_lr_p, random_state=42)
#####
...
if simple_run:
    t1 = time.time()
    model_m.fit(X_train1_m, y_train1_m)
    model_p.fit(X_train1_p, y_train1_p)

    visualize_tree_motions = False
    if visualize_tree_motions:
        dot_data_m = export_graphviz(model_m, out_file = None, class_names=['okay', 'paper', 'rock', 'scissors'], filled = True, rounded = True, special_characters = True)
        graph_m = pydotplus.graph_from_dot_data(dot_data_m)
        Image(graph_m.create_png())
    
    visualize_tree_particles = True
    if visualize_tree_particles:
        dot_data_p = export_graphviz(model_p, out_file = None, feature_names=col_names_p, class_names=['kaon', 'pion', 'positron', 'proton'], filled = True, rounded = True, special_characters = True)
        graph_p = pydotplus.graph_from_dot_data(dot_data_p)
        display(Image(graph_p.create_png()))
...
if test_criterion:
    print('heya')
    test_parameter = 'Splitting Criterion'
    n_range = ['entropy', 'gini'] 
    ...
    for n in n_range:
        ...
        model_m = AdaBoostClassifier(DecisionTreeClassifier(criterion = n, max_depth = best_max_depth_m),n_estimators = best_estimators_m, learning_rate = best_lr_m, random_state=42)
        ...
        model_p = AdaBoostClassifier(DecisionTreeClassifier(criterion = n, max_depth = best_max_depth_p),n_estimators = best_estimators_p, learning_rate = best_lr_p, random_state=42)
...
test_n_estimators = False
if test_n_estimators:
    test_parameter = 'Number of Estimators'
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = best_max_depth_m),n_estimators = n, learning_rate = best_lr_m, random_state=42)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = best_max_depth_p),n_estimators = n, learning_rate = best_lr_p, random_state=42)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
if test_learning_rate:
    ...
    n_range = [0.1*(i+1) for i in range(25)]
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = best_max_depth_m),n_estimators = best_estimators_m, learning_rate = n, random_state=42)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = best_max_depth_p),n_estimators = best_estimators_p, learning_rate = n, random_state=42)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
if test_max_depth:
    test_parameter = 'Weak Learner Tree Depth'
    n_range = range(2, 30, 1) 
    ...
    for n in n_range:
        ...
        t1 = time.time()
        print("looking at {} = {} on Motions Set".format(test_parameter, n))
        model_m = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = n),n_estimators = best_estimators_m, learning_rate = best_lr_m, random_state=42)
        model_m.fit(X_train_m, y_train_m.values.ravel('C'))
        y_pred_m = model_m.predict(X_val_m)
        scores_m[n] = accuracy_score(y_val_m, y_pred_m)
        scores_list_m.append(scores_m[n])
        ...
        t1 = time.time()
        print("looking at {} = {} on Particles Set".format(test_parameter, n))
        model_p = AdaBoostClassifier(DecisionTreeClassifier(criterion = 'gini', max_depth = n),n_estimators = best_estimators_p, learning_rate = best_lr_p, random_state=42)
        model_p.fit(X_train_p, y_train_p.values.ravel('C'))
        y_pred_p = model_p.predict(X_val_p)
        scores_p[n] = accuracy_score(y_val_p, y_pred_p)
        scores_list_p.append(scores_p[n])
...
if show_validation_performance:
    ...
    best_m = AdaBoostClassifier(DecisionTreeClassifier(max_depth = best_max_depth_m),n_estimators = best_estimators_m, learning_rate = best_lr_m, random_state=42)
    ...
    best_p = AdaBoostClassifier(DecisionTreeClassifier(max_depth = best_max_depth_p),n_estimators = best_estimators_p, learning_rate = best_lr_p, random_state=42)
...
if learning_curves:
    plot_learning_curves(model_m, X_train1_m, y_train1_m, low_limit=0.8, title = "Decision Tree - Motions Set - Post-Tuning Learning Curves")
    plot_learning_curves(model_p, X_train1_p, y_train1_p, low_limit=0.8, title = "Decision Tree - Particles Set - Post-Tuning Learning Curves")
